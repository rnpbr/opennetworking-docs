...
In this guide, you will learn how to configure a discovery rule in Zabbix to automatically identify network assets using the **SNMPv2** protocol, and create an **auto-registration action** to add discovered devices to monitoring.
...
1.  **Zabbix properly installed** on the `br-lab` network.
    If you have not yet done the installation, follow the [Zabbix Installation Guide](../../../pt/Ferramentas/Zabbix/index.md).
...
*   **Discovery Rule**: Defines the IP range to be scanned, the type of check (e.g. SNMP, ICMP), and how to extract the host name.
...
> ✅ When configured correctly, the rule will scan the entire `172.10.10.0/24` subnet every minute, looking for devices that respond to SNMP on port 161 with the community `public`.
...
Clear! Here is the enhanced excerpt in an academic and technical style, with clear formatting, complete explanations, and the inclusion of the sample images section to facilitate visual understanding:

!!! info "Note"
    Remember to activate the rule for it to work correctly. You can do this in the discovery rule configuration tab by checking the "Enable" option.
---
...
A Zabbix **discovery action** is a set of instructions that define **what should be done automatically after a host is detected** by a *Discovery Rule*. This mechanism allows the automation of infrastructure management, avoiding manual device registration.
...
With this action configured, every device detected on the `br-lab` network with SNMP support will be automatically added to Zabbix, with the defined template and monitoring group, allowing scalability and agility in infrastructure management.