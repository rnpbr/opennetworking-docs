...
Before you start using the monitoring tools, it's crucial to configure the communication base between your labs. For this, we use a dedicated Docker network called **br-lab**. The **br-lab** network facilitates the integration of analysis tools with the labs created in Containerlab, eliminating the need to repeatedly configure each tool for different labs.
...
After configuring the **br-lab** network, you can select the tool that best suits your needs. Each of the tools listed below offers specific functionalities for different types of monitoring, whether it's network devices, traffic, or logs.
...
These fixed IPs ensure stable and facilitated communication between the tools and the labs configured in **Containerlab**. This allows you to efficiently integrate multiple monitoring tools without having to redefine network configurations for each new lab.
