...
`nrx` is the command-line interface for NetReplica that allows you to configure and execute tasks directly without the need to create a configuration file. The following are the main commands you can use:
...
The `.conf` configuration file provides a structured way to define the variables necessary for topology export. It is particularly useful when you want to reuse the same settings or when there are many options to define.
...
1. **NB_API_URL**
    ...
    - **Usage**: Defines the URL to which NetReplica should send API requests. If you are using a local NetBox instance, change it to `'http://localhost:8000'`.
...
14. **EXPORT_CONFIGS**
    ...
    - **Example**: `true`
    - **Usage**: If `true`, the device configurations will be included in the export.

### Device Role Levels

15. **[DEVICE_ROLE_LEVELS]**
    - **Description**: Defines the viewing levels for different device roles.
    - **Example**:
      ```bash
      unknown =              0
      server =               0
      ...
      router =               4
      ```
    - **Usage**: Defines the viewing order of devices in the export. Devices with higher levels are displayed more prominently.
