...
To install the plugin in Netbox, we need to modify and add some files that are responsible for the configuration of Netbox.
...
```
!!! tip "Information"
    We changed the repository branch to have access to version 4.1.11 of Netbox.
...
- Changing the dockerfile to `Dockerfile-Plugins`, created earlier.
...
To test the Napalm plugin, we need devices for querying, for this, we will use the Discovery laboratory [More information](../../../../Laboratórios/Lab%20Descoberta/index.md). Therefore, add at least one device to Netbox.
...
2. Fill in:
   - **Name**: `Juniper`
   - **Slug**: `juniper` (or generated automatically)
3. Click on **Create**.
...
This functionality displays the Layer 2 neighbors detected via the LLDP (Link Layer Discovery Protocol) protocol. This feature allows you to automatically identify devices directly connected to the monitored equipment.
...
- `Candidate Config` (when supported by the operating system): Configuration being edited that has not yet been applied. Present in equipment that works with staging configurations (e.g. Juniper).

![Image of the Config tab](../../../../../img/netbox_imgs/napalmConfigPage.png)

:simple-target: **Purpose**:

This tab is essential for managing and tracking changes in the configuration of devices, as well as providing a practical and secure way to validate whether the persistent configurations are in accordance with those running - all within the NetBox interface.