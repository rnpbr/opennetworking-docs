# :material-power-plug-outline: Installing Diode Server
...
Let's modify the `.env` variables file and create a new `docker-compose.override.yml` file to override the docker-compose configurations.
...
This file is responsible for changing the `docker-compose` configurations by overriding the same's configurations.
...
With the Diode server properly configured, now it's time to put it into execution.
...
- Check that all services are in UP status
```bash
docker compose ps
```
```bash
