# :material-power-plug-outline: Installing Diode Client

The Diode Client is responsible for sending the collected data to the Diode Server using gRPC/Protobuf, enabling the subsequent ingestion of this information into NetBox.

There are different ways to use the Client. This documentation will present the use of the Orb Agent, developed by NetBox Labs. This agent not only performs automated discovery of networks and devices, but also offers observability functionalities over the monitored equipment.
...
## :material-arrow-down-box: **2. Files Needed for Installation**
...
Now, let's create the necessary files for Orb-Agent installation.
...
The agent needs to be in the same network as the devices to be imported, as we are in a docker environment, we will add it to the default laboratory network `br-lab`.

### :fontawesome-solid-square-root-variable: **2.2. `.env`**
The file that contains the variables responsible for Orb-Agent configuration.
...
# API Key for connection to Diode Server -> diode-ingestion
...
SCHEDULE='"*/10 * * * *"' # Time for the collection to be performed again
```
...
In Orb-Agent, configurations are defined through the `agent.yaml` file. It is in this file that we configure the connection to the **Diode Server**, specify the location for importing variables or access credentials, and define the types of discovery that the agent should execute.
...
The special `common` subsection, within the backends section, defines settings that are shared between all backends. Currently, this section allows passing the connection settings to the Diode server to all backends in a centralized manner.
...
The `policies` section defines which discovery policies should be assigned to each backend. Each policy describes specific settings for the type of discovery, such as scheduling, default properties, and the scope (targets).
...
After understanding a bit of the Orb-Agent structure, let's go to a practical example with a simple template that can be used to map the docker network `br-lab` and some devices in it. In this file we will use the backends: `network_discovery` and `device_discovery`.
...
With the Orb-Agent properly configured and running, you have a powerful automation and active discovery tool in your network. It allows the continuous and structured collection of data about devices and infrastructure, sending this information directly to NetBox via Diode Server, in a secure and flexible way.
...
In the next steps, you can expand discoveries, refine specific policies for different backends, and leverage the potential of NetBox as a source of truth for your infrastructure.