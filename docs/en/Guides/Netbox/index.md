...
4. On your local computer, open your web browser and go to:
    ...
    You will be redirected to NetBox running on the remote server through the SSH tunnel. You can now securely access NetBox.
...
For importing, you must organize the files in a correctly formatted CSV format, containing the fields indicated below. In addition, it is important that the import follows the order of the numbering of the files as shown in the data preparation.
...
- After clicking on one of the circuits, the circuit settings are similar to that shown in Figure 3. The terminations must be edited by clicking on the `Edit` icon as highlighted in the image below. By clicking on the icon, Netbox forwards you to the Cables section in Connections, as shown in Figure 4.
...
- Figure 4 presents the creation of connection cables, the numbering of cables only follows the order of creation. The `Side*` and `Interface*` items must be filled in to finalize the configuration of step 7.

![Figure 4: cable configuration screen for connection for the connection between devices.](../../../img/netbox_imgs/circuitCable.png)
**Figure 4:** cable configuration screen for connection for the connection between devices.
