...
If your environment is not configured, follow the steps [Configuration Guide](../../Getting%20Started.md)

   
### :material-application-import: 2.2 Importing Template to Netbox:
Containerlab uses [startup-config](https://containerlab.dev/manual/nodes/#remote-startup-config ) to import configurations to equipment, these are defined in templates within Netbox.
...
2. **Import the OSPF lab template:**
    ...
    !!! warning "Attention"
        Pay attention to the device image, as configuration templates are created in different ways for each type of image
...
```
!!! warning "Attention"
    Devices need time to be initialized and configured, the lab (graphite) may already be available and you cannot access the devices. </br>
!!! info "Tip"
...
- **Access via SSH to routers**: Use an SSH client to connect to the routers using the IP address of each device or the name assigned by Containerlab, along with the login credentials provided. The management IP address of each router is displayed after the lab is executed by Containerlab.
...
- **Access to Graphite (Topology Server)**: Access Graphite through the browser using the configured IP address and port. You can also access it directly by clicking on the SSH icon on the desired router.
...
- **SNMP Monitoring**: Use SNMP-compatible tools to monitor network health and performance metrics.