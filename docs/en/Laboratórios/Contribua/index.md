...
Contributing to the documentation of labs in **Containerlab** is a simple process, but it is important to follow a few steps to ensure that all links and functionalities work correctly. This page provides the necessary instructions to document a lab, as well as to configure the essential scripts for the proper functioning of the deployment process.
...
In the documentation, it's important for labs to have images representing the network topology of the lab. To do this, follow the [Guide to Creating Images to Represent Labs](Guia Criando Imagens para Representar Laboratórios.md).

### Steps to Document the Lab

Here are the main steps to document your lab:
...
To use the documentation template, simply access **docs/pt/Laboratórios/Contribua/Template lab.md**, create a copy for the laboratories folder, name it after your lab, and fill in the information.

---

## 2. Scripts for Quick Download

To ensure that the lab's download and deployment process works correctly, it is necessary to include two essential scripts within the lab's folder in GitLab. These scripts will allow the lab to be downloaded and configured automatically with a simple command.

### 2.1 Script `get.sh`

The `get.sh` script is used on Linux/Mac systems to automatically download and unpack the lab. It checks if the `tar` command is installed, and if it is not, it installs the necessary package before downloading the file.
...
With these steps, you will be able to contribute effectively to the lab's documentation and ensure that all download links and scripts are working correctly. without changing the structure of the documentation, adding anything, or altering the links or references.