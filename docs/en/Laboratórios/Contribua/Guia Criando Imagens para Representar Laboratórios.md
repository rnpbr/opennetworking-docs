...
This guide teaches you how to create images representing network topologies for the labs, using the [Draw.io](http://draw.io/) template provided in the repository. The images must follow the project's visual identity and be saved in **SVG** format within the correct folder.

---

### 1. **Opening the Template**

In the root of the `docs/Templates` repository, you will find a [**Draw.io**](http://draw.io/) template. You can see it below. Open the file and you will see that it already contains all the necessary components to create network topologies, including:
...
1. **Add Network Components**: Copy and paste the **routers**, **switches**, and **servers** icons from the template's components area.
...
2. **Add IP Property**:
    ...
    - Click on **Add Property**, put the name as **ip**, and add the IP address of the interface.
...
After finishing the topology, save the image in **SVG** format:
...
After saving the image, update the Markdown file of the documentation, including the topology image:
