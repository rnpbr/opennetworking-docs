...
The main objective of the `elk-lab` lab is to simulate the sending and analysis of IPFIX traffic flows in a topology of three interconnected routers (GO, MS, and MT), using OSPF for dynamic routing and Elastic Stack tools for real-time observability and traffic analysis.
...
* The external network `br-lab` connects the network elements to the ELK stack.
...
Below are listed the minimum hardware and software requirements needed to run the lab. Be sure to include essential tools such as **Containerlab** and **Docker**, as well as the previously created `br-lab` network.
...
To configure data collection using IPFIX, with our guide to [IPFIX configuration](../../Ferramentas/Elasticsearch/Configurando IPFIX no ELK.md).
without changing the structure of the documentation, adding anything, or altering links or references.