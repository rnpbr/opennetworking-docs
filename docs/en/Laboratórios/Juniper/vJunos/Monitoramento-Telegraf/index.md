...
The `telegraf-lab` lab has the main objective of simulating traffic monitoring of a topology with three interconnected routers (GO, MS, and MT), using dynamic routing protocols (OSPF), flow export (IPFIX), and monitoring with SNMP. The collection and visualization of network metrics are performed with modern observability tools: **Telegraf**, **InfluxDB**, **Chronograf**, and **Grafana**.
...
And have the telegraf stack previously installed, to learn more about zabbix installation access: [Instalação do Telegraf](../../../../Ferramentas/Telegraf/index.md)
...
Time series database where Telegraf metrics are stored. It can be accessed via port 8086.
