...
- `binds` allows you to map volumes, such as interface mappings.
- `include` is used to include additional labels from the CLAB template.

### Creating CLAB Templates

To create your own CLAB templates, follow the structure and example format provided. You can create `.j2` files, remembering that the file name should be the platform name for each device type, and customize the configurations as needed. Save your templates to the `templates/clab/kinds/` directory of your NetReplica environment.
...
Once you have created and configured the `.conf` file with the desired options, you are ready to run NetReplica and start replicating and analyzing networks based on NetBox information. Follow the steps below to run NetReplica:

1. **Configuration File Location**: Make sure the `.conf` file is in the correct location. If you are using NetReplica via Docker, remember that the `.conf` file must be within the "conf" directory that is created after running the compose.
...
3. **Running NetReplica**: Execute the command to start NetReplica, passing the path to the `.conf` file as an argument. For example, if the `.conf` file is in the current directory and is called `netreplica.conf`, the command might be:
...
4. **Monitoring Execution**: During execution, NetReplica will use the settings in the `.conf` file to access NetBox and begin network replication and analysis. Monitor the terminal output to see the progress and any relevant messages.
5. **Execution Results**: After execution is complete, the replication and analysis results will be available in the output directory specified in the `.conf` file (usually the `OUTPUT_DIR` directory). You can find the exported files, such as images, device configurations, and other data, according to the defined settings.
