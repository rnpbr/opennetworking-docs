...
4.  Access the `.env` environment file from the example provided:

    ```
    cd env
    ```

5.  Edit the `.env` file to adjust the settings as needed. Specifically, you can change database credentials, passwords, email settings, etc., according to your preferences and requirements. By default, it is all configured for quick execution, but it is highly recommended to change this information if you want to put it into production.
6.  If you want to change the Netbox version, edit the image version in the compose file.
...
7.  Build and run the Docker containers:
    ...
    **Note:** The communication port used may differ from port 8000 if another active application uses this port. In this case, check for another available port (for example: 8080 or 8081) and be sure to replace it appropriately in the following steps.
...
After completing the installation of NetBox on Docker, you can access it via a web browser. By default, NetBox will be available locally at [http://localhost:8000/](http://localhost:8000/). However, if you want to access NetBox securely through an SSH tunnel, follow the steps below:
...
For the import, the files must be organized in a correctly formatted CSV format, containing the fields indicated below. In addition, it is important that the import follows the order of the file numbering as shown in the data preparation.
...
2.  **Find the import option**: Check the section related to the data you want to import, and look for an import **icon** as we see in Figure 1 below:
    ...
    ![**Figure 1**: by clicking on the import icon you can upload the CSV file.](../../../img/netbox_imgs/import.png)
    **Figure 1**: by clicking on the import icon you can upload the CSV file.
...
*   After clicking on one of the circuits, the circuit settings are similar to that shown in Figure 3. The terminations must be edited by clicking on the `Edit` icon as highlighted in the image below. By clicking on the icon, Netbox forwards to the Cables part in Connections, as shown in Figure 4.
...
*   Figure 4 presents the creation of the connection cables, the numbering of the cables follows only the creation order. The `Side*` and `Interface*` items must be filled in to finalize the configuration of step 7.

![Figure 4: cable configuration screen for connecting between devices.](../../../img/netbox_imgs/circuitCable.png)
**Figure 4:** cable configuration screen for connecting between devices.
