...
4. Access the environment files `.env` from the provided example:
    
    ```
    cd env
    ```
    
5. Edit the `.env` file to adjust the settings as needed. Specifically, you can change database credentials, passwords, email settings, etc., according to your preferences and requirements. By default, it's all configured for quick execution, but it is highly recommended to change this information if you intend to put it into production.
6. If you want to change the NetBox version, edit the compose file in the image version section.
**Note:** Changes between distant versions may cause errors. Look for the correct versions at [https://github.com/netbox-community/netbox-docker/releases](https://github.com/netbox-community/netbox-docker/releases)
7. Build and run the Docker containers:
    ...
    **Note:** The communication port used may differ from port 8000 if another active application uses this port. In this case, check for another available port (e.g., 8080 or 8081) and don't forget to replace it appropriately in the following steps.
    
8. Wait for the containers to be built and initialized.
...
11. To create the administrator user, run the following command in the terminal after the containers are initialized:
...
4. On your local computer, open your web browser and go to:
    ...
    You will be redirected to NetBox running on the remote server through the SSH tunnel. Now you can access NetBox securely.
    

Remember that the SSH tunnel will keep the connection active while the remote server's terminal is open. If you want to keep the tunnel running in the background, add the `-f` option to the SSH command in step 9:

```
ssh -f -N -L 8080:localhost:8000 user@server_address
```

With this, you will be able to access NetBox securely through an SSH tunnel, ensuring the protection of your data during transmission.

## 3. Import

For importing, you must organize the files in correctly formatted CSV format, containing the fields indicated below. In addition, it is important that the import follows the order of the file numbering as indicated in the data preparation.

### 3.1. Data Preparation

The import of the csv files must follow the established numbering and contain the indicated information. The names (**in bold**) indicate the import locations, and the information below (*in italics*) indicates the fields required for import.
...
2. **Find the import option**: Check the section related to the data you want to import, and look for an **import icon** as shown in Figure 1 below:
...
To do this, follow the instructions below to complete the configuration. Click on the created circuit and on the `edit` icon as highlighted in the image below.
...
- After clicking on one of the circuits, the circuit settings are similar to those presented in Figure 3. The terminations must be edited by clicking on the `Edit` icon as highlighted in the image below. By clicking on the icon, Netbox forwards to the Cables section in Connections, as shown in Figure 4.
...
- Figure 4 presents the creation of the connection cables, the numbering of the cables follows only the order of creation. The `Side*` and `Interface*` items must be filled in to finalize the configuration of step 7.

![Figure 4: screen for configuring the connection cables for connecting the devices.](../../../img/netbox_imgs/circuitCable.png)
**Figure 4:** screen for configuring the connection cables for connecting the devices.
