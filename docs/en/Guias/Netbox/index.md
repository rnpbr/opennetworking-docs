...
5. Edit the `.env` file to adjust the settings as needed. Specifically, you can change database credentials, passwords, email settings, etc., according to your preferences and requirements. By default, it is fully configured for quick execution, but it is highly recommended to change this information if you want to put it into production.
6. If you want to change the NetBox version, edit the compose file in the image version section.
**Note:** Changes between distant versions can cause errors, look for the correct versions at [https://github.com/netbox-community/netbox-docker/releases](https://github.com/netbox-community/netbox-docker/releases)
7. Build and run the Docker containers:
    ...
    **Note:** The communication port used may differ from port 8000 if another active application uses this port. In this case, check for another available port (for example: 8080 or 8081) and don't forget to replace it appropriately in the following steps.
...
10. You will be prompted to set the administration password.
11. To create the administration user, run the following command in the terminal after the containers are initialized:
...
After completing the installation of NetBox in Docker, you can access it via a web browser. By default, NetBox will be available locally at [http://localhost:8000/](http://localhost:8000/). However, if you want to securely access NetBox through an SSH tunnel, follow the steps below:
...
4. On your local computer, open your web browser and go to:
    ...
    You will be redirected to the NetBox that is running on the remote server through the SSH tunnel. You can now access NetBox securely.
...
For the import, you must organize the files in a correctly formatted CSV format, containing the fields indicated below. In addition, it is important that the import follows the order of the file numbering as stated in the data preparation.

### 3.1. Data Preparation

The import of CSV files must follow the established numbering and contain the indicated information. The names (**in bold**) indicate the import locations, and the information below (*in italics*) indicates the fields needed for the import.
...
![**Figure 2**: click on the circuit (arrow on the left), then select the Circuit ID (detached and indicated with the arrow) to perform the configuration.](../../../img/netbox_imgs/circuitID.png)
**Figure 2**: click on the circuit (arrow on the left), then select the Circuit ID (detached and indicated with the arrow) to perform the configuration.

- After clicking on one of the circuits, the circuit configurations are similar to those presented in Figure 3. The terminations must be edited by clicking on the `Edit` icon as highlighted in the image below. By clicking on the icon, Netbox forwards to the Cables section in Connections, as shown in Figure 4.
...
💡 Remember to adapt the subcategories and import locations according to the specific functionalities of your NetBox. Each category may have unique fields and settings for import.
 ...
 without changing the structure of the documentation. and without adding anything, and without changing the links or references.