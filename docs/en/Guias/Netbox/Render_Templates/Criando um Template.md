...
4. **Automated Rendering:** When an administrator creates or updates a device in NetBox, "Render Template" comes into play. NetBox identifies the device type and the corresponding configuration template.
...
7. **Configuration Application:** The generated configuration can be deployed to the device using traditional methods, such as SSH/SCP file transfer, or integrated with configuration management tools for additional automation.
...
- **{{ }} - J2 Variables:** Expressions between double curly braces, such as `{{ device.name }}`, are J2 variables. They are replaced with the actual device values during template rendering. For example, `{{ device.name }}` will be replaced by the specific device's name.
...
Now that we've explained how it works, here are the available variables for devices registered in NetBox
