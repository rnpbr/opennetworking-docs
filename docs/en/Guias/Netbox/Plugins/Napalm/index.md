...
This file contains a list of Netbox plugins (as PyPO Python packages) that must be installed during the Docker image build.
...
PLUGINS_CONFIG = {
    "netbox_napalm_plugin": {
        "NAPALM_USERNAME": "admin", # User for accessing equipment
        "NAPALM_PASSWORD": "admin@123", # Password for accessing equipment
    },
}
```
...
To test the Napalm plugin, we need devices to consult, for this, we will use the Discovery laboratory [More information](../../../../Laboratórios/Lab%20Descoberta/index.md). Therefore, add at least one device to Netbox.
...
The Status tab, enabled through the integration of NetBox with the NAPALM plugin, displays real-time operational information from the network device. This functionality allows the administrator to quickly monitor the current state of the equipment without leaving the NetBox interface.
...
This feature displays Layer 2 neighbors detected via the Link Layer Discovery Protocol (LLDP). This functionality allows you to automatically identify devices directly connected to the monitored equipment.

:material-pin: **Important Behavior**:

- Neighbor viewing is limited to interfaces previously registered in NetBox.
...
This tab is essential for managing and tracking changes in device configuration, as well as providing a practical and secure way to validate whether persistent settings are consistent with those running — all within the NetBox interface.