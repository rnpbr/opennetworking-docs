...
In the documentation, it's important for labs to have images representing the network topology of the lab. To do this, follow the [Guide Creating Images to Represent Labs](Guia Criando Imagens para Representar Laboratórios.md).

### Steps for Documenting the Lab
...
5. **Deployment Instructions**:
    - Document the lab deployment, whether it's the ready-made or customized version. Be sure to follow the deployment guides with appropriate links.
...
To ensure that the lab download and deployment process works correctly, it is necessary to include two essential scripts within the lab's folder in GitLab. These scripts will allow the lab to be downloaded and configured automatically with a simple command.
...
1. **Dependency Check**: The script checks if the `tar` package is installed. If not, it tries to install it using the system's package manager (`apt-get`, `dnf`, `yum`, or `pacman`).
...
## 3. Making Sure the Scripts are Correct
...
With these steps, you can effectively contribute to the lab documentation and ensure that all download links and scripts are working correctly, without altering the structure of the documentation, adding anything, or changing the links or references.