...
This lab simulates a network with 3 routers in a ring topology, configured using OSPF for dynamic routing and SNMP for sending telemetry over the network.

## :material-lan: **1. Topology and Configurations**

The topology consists of three routers (PB, PE, JPA) connected in a ring. Each router is configured with network interfaces and IP addresses, in addition to OSPF protocols for routing and SNMP for monitoring, as we can see in the following image.
