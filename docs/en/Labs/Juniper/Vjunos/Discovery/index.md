...
The objective of this lab is to import network devices and their configurations into Netbox from Zabbix, in addition to demonstrating the basic operation of OSPF routing between two routers connected in a ring and monitored via SNMP.
...
- **SNMP (Simple Network Management Protocol)**: Used for monitoring and managing the network, allowing access to device telemetry information.
---
...
- **NetDevOps and Inventory Management Training:** Excellent for training professionals in modern network operations practices, focusing on tool integration via API and infrastructure documentation automation.

- **Centralized Device Repository Creation:** Enables building a reliable database about the network in real-time, from information discovered via SNMP and documented in Netbox.

- **Zabbix + Netbox API Integration Study:** Allows exploring the use of RESTful APIs to synchronize information between monitoring and network asset management tools.

- **SNMP Discovery and Centralized Management Teaching:** Provides a practical experience for students and professionals to understand how network data collection (interfaces, IPs, manufacturers, etc.) occurs and how this data is handled by management tools.
...
# Devices
DEVICE_USERNAME=admin                         # Default username to access the routers
DEVICE_PASSWORD=admin@123                     # Default password to access the routers
```
5. Now, to import the routers into Zabbix, execute the command:
```bash
...
!!! warning "Attention"
    Before accessing, check the log of a device to verify that it was started and configured correctly.
---
...
✅ Done! Your environment is now configured, monitored, and documented in Netbox. Feel free to customize or expand the topology according to the objectives of your study or project, without altering the structure of the documentation, adding anything, altering the links or references.