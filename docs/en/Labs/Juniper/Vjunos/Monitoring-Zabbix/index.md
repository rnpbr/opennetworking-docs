# :material-bookmark: Juniper vJunos Monitoring with Zabbix
...
The topology of this lab consists of two routers interconnected by a point-to-point /31 network, allowing direct communication between them. The routers are configured with OSPF to ensure dynamic routing between interfaces. Network monitoring is performed through an external network called br-lab, where routers are connected via virtual interfaces. Through this configuration, Zabbix is able to monitor the network's connectivity and performance.
...
!!! warning "Attention"
    Check if your processor has **hardware virtualization support** and if this functionality is **enabled in the BIOS/UEFI**.
...
!!! tip "Tip"
    The ready-made deployment is useful for those who want to start quickly with a configured environment.
